...
public class PuzzleBord : MonoBehaviour
{
    ...
    private Piece CreatePiece(Vector2Int gridPosition, PieceType type)
    {
        ...
        Piece piece = Instantiate(piecePrefab, worldPosition, Quaternion.identity, transform);
    ...
    private void UpdateActiveWorldPositions()
    {
        ...
        activePair.PivotPiece.transform.position = GridToWorld(activePair.Pivot);
        activePair.ChildPiece.transform.position = GridToWorld(activePair.Pivot + activePair.Offset);
    }
        ...
        for (int x = 0; x < width; x++)
        {
            ...
            for (int y = 0; y < height; y++)
            {
                ...
                if (writeRow != y)
                {
                    ...
                    piece.transform.position = GridToWorld(new Vector2Int(x, writeRow));
                }
